...
func (s *InterceptSuite) TestIntercept() {
	...
	_, proxyIP := s.startProxy(t, []string{"bridge", "--log-paths", "stderr", "server", "--addr", ":9090"})
...
func (s *InterceptSuite) TestIngress() {
	...
	proxyC, proxyIP := s.startProxy(t, []string{"bridge", "--log-paths", "stderr", "server", "--addr", ":9090", "-l", "8080/tcp"})
